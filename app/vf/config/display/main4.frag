...
#define MEDIA_BBOX_OVERFLOW_MODE 0 // 0 = debug (red), 1 = clamp edges, 2 = tiles, 3 = flipped tiles
...
struct Plot {
    ...
    float weight;
    float bulgeFactor;
    float reciprocalMediaBulgeFactor;
    bool debugFlag;
    vec2 bboxCenter;
};
...
//        p = (p - centerForceNCoords) * plot.reciprocalMediaBulgeFactor + centerForceNCoords;
        p = (p - centerForceNCoords) * plot.reciprocalMediaBulgeFactor * plot.reciprocalMediaBulgeFactor  + centerForceNCoords;
//    p = (p - plot.bboxCenter) * plot.reciprocalMediaBulgeFactor * plot.reciprocalMediaBulgeFactor  + plot.bboxCenter;
...
void applyMediaBboxBulge(inout vec2 cellNCoords, inout float reciprocalMediaBulgeFactor, in vec2 cellCoords, in float bulgeFactor) {
    ...
    if (MEDIA_BULGE == 1 || bMediaDistortion) {
        ...
        reciprocalMediaBulgeFactor = mix(1.0, bulgeSmoothstep(edge), bulgeStrength);

        #if BULGE_BLENDING == 1
            edge = length(cellCoords - centerForceCoords2) / bulgeRadius2;
            reciprocalMediaBulgeFactor = blendBulges(reciprocalMediaBulgeFactor,  mix(1.0, bulgeSmoothstep(edge), bulgeStrength2));
            edge = length(cellCoords - centerForceCoords3) / bulgeRadius3;
            reciprocalMediaBulgeFactor = blendBulges(reciprocalMediaBulgeFactor,  mix(1.0, bulgeSmoothstep(edge), bulgeStrength3));
        #endif
    }
...
void calcMediaBbox(in uint index, inout vec4 mediaBbox, in vec2 cellCoords, in float bulgeFactor, inout float reciprocalMediaBulgeFactor, in float edgeBorder, in float mediaWeightOffsetScale, inout vec2 bboxCenter) {
    vec2 midSum = vec2(0.0);
    vec2 nonNMidSum = vec2(0.0);
    vec2 nonBulgeMidSum = vec2(0.0);
...
//        #if BULGE == 1
//            vec2 neighborCoords = fetchCellCoords(neighborIndex);
////            float edge = length(neighborCoords - centerForceCoords) / bulgeRadius;
////            float reciprocalMediaBulgeFactor2 = mix(1.0, bulgeSmoothstep(edge), bulgeStrength);
////            neighborNCoords = (neighborNCoords - centerForceNCoords) * reciprocalMediaBulgeFactor2 + centerForceNCoords;
////            neighborNCoords = (neighborNCoords - centerForceNCoords) * reciprocalMediaBulgeFactor + centerForceNCoords;
//
//            applyMediaBboxBulge(neighborNCoords, reciprocalMediaBulgeFactor, neighborCoords, bulgeFactor);
//        #endif
        ...
        #if BULGE == 1
        vec2 neighborCoords = fetchCellCoords(neighborIndex);
        nonNMid = mix(cellCoords, neighborCoords, midFactor);
        nonNMidSum += nonNMid;
        nonBulgeMidSum += mid;
        applyMediaBboxBulge(mid, reciprocalMediaBulgeFactor, nonNMid, bulgeFactor);
    ...
    vec2 nonNMidAvgCenter = nonNMidSum / float(neighborsLength);
    vec2 nonBulgeMidAvgCenter = nonBulgeMidSum / float(neighborsLength);

    #if BULGE == 1
    applyMediaBboxBulge(nonBulgeMidAvgCenter, reciprocalMediaBulgeFactor, nonNMidAvgCenter, bulgeFactor);
//    avgCenter = nonBulgeMidAvgCenter;
    #endif


    //    avgCenter = (avgCenter - centerForceNCoords) * reciprocalMediaBulgeFactor + centerForceNCoords;
...
//        offset /= reciprocalMediaBulgeFactor;
...
//    applyMediaBboxBulge(cellNCoords, reciprocalMediaBulgeFactor, cellCoords, bulgeFactor);
    ...
    bboxCenter = avgCenter;

//    mediaBbox.xy = (mediaBbox.xy - centerForceNCoords) / reciprocalMediaBulgeFactor / reciprocalMediaBulgeFactor + centerForceNCoords;
//    mediaBbox.zw = (mediaBbox.zw - centerForceNCoords) / reciprocalMediaBulgeFactor / reciprocalMediaBulgeFactor + centerForceNCoords;
...
void sortClosest(
    ...
    float d = dist(p, fetchCellCoords(index), weight, weightOffset);
...
Plot init(vec2 p) {
    ...
    return Plot(indices, uvec4(uint(-1)), vec2(0.), 0., vec4(0.), 0., 0., 1., 1., false, vec2(0.));
}
...
//    #endif
    ...
    vec2 bboxCenter = vec2(0.);
    #if MEDIA_ENABLED == 1
        calcMediaBbox(index, mediaBbox, cellCoords, bulgeFactor, reciprocalMediaBulgeFactor, edgeStepStart*2., mediaWeightOffsetScale, bboxCenter);
    #endif
    ...
    return Plot(indices, indices2, edge, edgeStep, mediaBbox, cellScale, weight, bulgeFactor, reciprocalMediaBulgeFactor, debugFlag, bboxCenter);
}
