...
//    #endif
    ...
    vec2 center = mix(mediaBbox.xy, mediaBbox.zw, 0.5);
    if (abs(p.x - center.x) < 0.025 && abs(p.y - center.y) < 0.025) {
        c = vec3(1.,0.,0.);
        return;
    }
...
//    float reciprocalMediaBulgeFactor2 = reciprocalMediaBulgeFactor;
...
//            float reciprocalMediaBulgeFactor2 = 1. / mix(1.0, bulgeSmoothstep(edge), bulgeStrength);
...
//        midFactor /= reciprocalMediaBulgeFactor2;
...
//    applyMediaBboxBulge(avgCenter, reciprocalMediaBulgeFactor, avgCenter, bulgeFactor);
...
//    mediaBbox.xy = cellNCoords - offset;
//    mediaBbox.zw = cellNCoords + offset;
}
